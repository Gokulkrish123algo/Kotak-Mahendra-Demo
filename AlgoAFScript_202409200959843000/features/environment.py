...
def after_scenario(context, scenario):
    try:
        if (context.recording1.upper() == "TRUE" or \
                context.recording2.upper() == "TRUE"):
            stop_recording(context)
        if context.recording1.upper() == "TRUE" and \
                context.recording2.upper() == "FALSE":
            stop_recording(context)
        elif context.recording1.upper() == "FALSE" and \
                context.recording2.upper() == "TRUE":
            if scenario.status == 'passed':  # Scenario passed
...
def start_recording(context):
    try:
        ...
        context.video_path = os.path.join(os.getcwd(), context.ReportFolder, f"scenario_{name}.mp4")
...
def stop_recording(context):
    try:
        if hasattr(context, 'is_recording') and context.is_recording:
            ...
            with open(context.video_path, "wb") as video_file:
                video_file.write(base64.b64decode(raw_video_data))
    except Exception as e:
